...
void print_snapshots(order_book_manager& obm, int step) {
    SnapshotTOB tob = {};
    obm.snapshot_tob(tob, 1, step, 123456789 + step, 1, 101.5, 1000);
    ...
    SnapshotFOD fod = {};
    obm.snapshot_fod(fod, 1, step, 123456789 + step, 1, 101.5, 1000);
...
int main() {
    ...
    for (auto& ev : events) obm.process_event(&ev);
    print_snapshots(obm, step++);

    // Cancel one bid at 100.0 (id 1)
    Event cancel_bid1 = {1, 7, EVENT_TYPE_CANCEL, 0, 1, 0, 0, 100.0, 1, 0, 0, 0, 0};
    obm.process_event(&cancel_bid1);
    ...
    obm.process_event(&add_bid4);
    obm.process_event(&add_bid5);
    ...
    obm.process_event(&cancel_bid3);
    obm.process_event(&cancel_bid7);
    obm.process_event(&cancel_bid8);
